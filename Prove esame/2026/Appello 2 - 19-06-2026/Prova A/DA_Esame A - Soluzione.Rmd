...
- **6.4** [**3 pt**]: Si ottenga il modello di regressione lineare avente come variabile risposta `Happiness_Score` e come variabili esplicative `Air_Quality_Index` e `Air_Quality_Index` al quadrato. Selezionare quale delle seguenti affermazioni è FALSA.
  ...
  a. `[V]` L'incremento del coefficiente di determinazione rispetto al modello al punto 6 è inferiore ad un punto percentuale. 
  b. `[F]` L'effetto di un incremento unitario di `Air_Quality_Index` su `Happiness_Score` è di circa -0.03.
  c. `[V]` Il valore predetto di `Happiness_Score` quando `Air_Quality_Index` è 5 è 8.24.
  d. `[V]` I residui mostrano un'asimmetria negativa.
...
- **Punto 7**: [**4 pt**]: Si applichi un algoritmo di cluster analysis gerarchico con il metodo di Ward che consideri le seguenti variabili: `Green_Space_Area`, `Air_Quality_Index`, `Noise_Level`, `Traffic`. Si selezionino due gruppi e si scelga la risposta VERA tra le seguenti.
  ...
  a. `[V]` Uno dei gruppi è caratterizzato principalmente da paesi in Asia ed Europa.
  b. `[F]` Rispetto alla variabile `Traffic`, le città con traffico alto non si trovano in un unico gruppo.
  c. `[F]` Le città con un livello di `Noise_level` basso sono concentrate tutte in un unico cluster.
